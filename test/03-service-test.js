...
(function() {
  ...
  describe('Tests to http-service', function() {
    ...
    // @ts-ignore
    const httpRouter = new HttpRouter(args);

    describe('General API', function() {
      
      it('should by defined', function() {
        assert.isFunction(HttpService);
        assert.isFunction(HttpService.constructor);
        // @ts-ignore
        assert.throw(() => HttpService());
        // @ts-ignore
        assert.instanceOf(new HttpService(new HttpRouter(args)), HttpService);
      });
    ...
    describe('HttpRouter: tests of properties', function() {
      ...
      // @ts-ignore
      const httpService = new HttpService(httpRouter);
      
      it('#doHttp', function() {
        assert.isTrue(Reflect.has(httpService, 'doHttp'), 'deve esta definido');
        expect(httpService.doHttp()).to.be.a('promise', 'deve retornar uma promise');
      });

      it('#doDelete', function() {
        assert.isTrue(Reflect.has(httpService, 'doDelete'), 'deve esta definido');
        expect(httpService.doDelete()).to.be.a('promise', 'deve retornar uma promise');
      });

      it('#doGet', function() {
        assert.isTrue(Reflect.has(httpService, 'doGet'), 'deve esta definido');
        expect(httpService.doGet()).to.be.a('promise', 'deve retornar uma promise');
      });

      it('#method', function() {
        assert.isTrue(Reflect.has(httpService, 'method'), 'deve estar definido');
        assert.equal(httpService.method, args.request.method, `deve retornar ${args.request.method}`);
      });
